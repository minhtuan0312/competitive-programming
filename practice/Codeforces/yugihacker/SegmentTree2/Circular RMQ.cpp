...
struct segment_tree{
    ...
    vector<ll> st, lazy;
    segment_tree() {}
    segment_tree(int n): n(n), st(n << 2, LLONG_MAX), lazy(n << 2, NO_LAZY) {}
    ...
    ll query(int v, int l, int r, int ql, int qr) {
        ...
        push(v);
        int m = (l + r) >> 1;
        ll q1 = query(v << 1, l, m, ql, min(qr, m));
...
int main(void) {
    ...
    while(q--) {
        ...
        int l = stoi(op[0]), r = stoi(op[1]);
        l++, r++;
        if(sz(op) == 2) {
            if(l > r) {
                ll q1 = seg.query(1, 1, n, l, n);
                ll q2 = seg.query(1, 1, n, 1, r);
                cout << min(q1, q2) << nl;
            } else {
